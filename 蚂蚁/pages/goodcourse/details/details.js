...
Page({
  ...
  pays() {
    ...
    if (user) {
      ...
      if (item.type == 1) { // 等于1 进入简易页面
        ...
        } else { // 详情表单页面和发起拼课页面相同
          console.log('itemitemitem', item)
          wx.navigateTo({
            url: '../../info/choice/choice?type=1&id=' + item.data.id + '&classdetail=' + JSON.stringify(item),
          })
        }
      }
    } else {
  ...
  // 请求购买
  getpay(type, is_pinke) { // type=2 is_pinke=1 
    console.log('getpaygetpaygetpay')
    let item = this.data
    let _this = this
    http.post('/indexinterface/confirm_purchase', {
      ...
      if (res.code == 0) {
        ...
        wx.navigateTo({
          url: '../../pink/pink?classdetail=' + JSON.stringify(this.data)
        })
        // wx.navigateTo({
